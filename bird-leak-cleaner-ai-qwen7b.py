#!/usr/bin/env python3
"""
Bird Leak Cleaner - Parser robusto de dados vazados usando Ollama
...
def create_prompt(lines: list[str]) -> str:
    """Cria o prompt para o modelo LLM."""
    lines_text = "\n".join([f"{i+1}. {line}" for i, line in enumerate(lines)])
    
    prompt = f"""Analise cada linha e extraia URL, Login e Senha.

IMPORTANTE:
- Cada linha pode ter: URL+Login+Senha, apenas Login+Senha, apenas URL+Login, ou apenas um deles
- Separadores comuns: ":", "|", ";", espaço - podem aparecer MÚLTIPLOS na mesma linha
- CUIDADO: esses caracteres (: | ; espaço) também podem fazer PARTE da senha - não remova!
- Se não conseguir identificar um campo, deixe vazio
- URLs: começam com http/https/ftp ou são domínios (site.com)
- Login: emails (@), usernames, CPFs, telefones
- Senha: geralmente o último campo, pode conter QUALQUER caractere
...
def validate_result(result: dict) -> bool:
    ...
    has_password = bool(result.get("password", "").strip())
    return has_login or has_url or has_password
